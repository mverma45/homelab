# Manny's Homelab

My hands-on Linux and DevOps homelab.

Currently learning
- Ubuntu Server
- SSH
- Docker
- NGINX
- Git and GitHub
- Linux Skills
- File management
- Permissions
- Linux Filesystems

## September 2026

### Intermediate Linux Skills
Continued studying linux from Udemy Linux Administration Bootcamp: Go from Beginner to Advanced

Learned about the Linux Directory Structure
linuxtrainingacademy.com
The Linux kernel is the core of linux

Redhat Linux and Ubuntu are the two big distributions used in Technology
Redhat Enterprise Linux also known as RHEL pronounced Rel

Who uses Redhat: Banks, Airlines, Telecoms, Healthcare, Government

You need a license to run redhat, to avoid paying for a licence you can use redhat enterprise Linux derivative AlamaLinux or Rocky Linux which are free. CentOs used to be free redhat Linux, it moved from being a clone of redhat to being more of a preview of what is coming to rehat
